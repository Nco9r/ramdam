        ...
        <div class="card" v-for="bsd in filterclient" :key="bsd.id">
          ...
          <div class="item">
            ...
            <div class="content_item">
              ...
              <p v-if="bsd.node.status === 'SIGNED_BY_PRODUCER'" class="statut">Signé par le producteur</p>
              <p v-if="bsd.node.status === 'SENT'" class="statut">Envoyé au destinataire</p>
              <p v-if="bsd.node.status === 'REFUSED'" class="statut">Refusé</p>
              <p v-if="bsd.node.status === 'PROCESSED'" class="statut">Traité</p>
            </div>
          </div>
...
export default {
  async asyncData({ $axios }) {
    const bsdasris = await $axios.$get(`${process.env.GET_TRACK}/api/getbsdasriall`)
    return { bsdasris }
  },
  data() {
    return {
